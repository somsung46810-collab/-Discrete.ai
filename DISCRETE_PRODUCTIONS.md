...
**Discrete.ai is defined as Discrete Productions:** a deterministic local production pipeline for triangulating duplicate (`DUPE`) and canonical (`DEDUPE`) values through R.E.P.L. — Read, Evaluate, Print, Loop — type buckets.
...
3. **Print** the classified entries into deterministic REPL buckets.
4. **Loop** by replaying the buckets in their fixed production order.
5. End with a structured `endif` status at **100% completion**.
...
```json
{
  "percentage": 100,
  "stage": "print",
  "message": "Printed 2 of 2 entries into REPL buckets",
  "condition": "endif"
}
```

Percentages are monotonic and clamped from `0` to `100`. The last event always uses `condition="endif"`.
